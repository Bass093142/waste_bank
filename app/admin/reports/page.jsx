...
function StatCard({ id, highlightId, icon: Icon, label, value, unit, colorClass, bgClass, loading }) {
  ...
  return (
    <div id={id} className={`bg-white dark:bg-slate-900 rounded-2xl shadow-sm border-l-4 ${colorClass} border-y border-r border-slate-100 dark:border-slate-800 p-4 flex items-center justify-between gap-3 transition-all duration-700 ease-in-out ${isHighlighted ? 'ring-[6px] ring-indigo-500 ring-offset-4 dark:ring-offset-slate-950 scale-[1.12] z-30 relative shadow-[0_0_40px_rgba(99,102,241,0.5)] bg-indigo-50/80 dark:bg-indigo-900/30' : 'scale-100 z-0'}`}>
      <div className="min-w-0">
        <p className="text-[10px] font-black text-muted-foreground uppercase tracking-wider mb-1 truncate">{label}</p>
        {loading ? <Skeleton className="h-8 w-28 rounded-lg dark:bg-slate-800" /> : <p className={`text-xl sm:text-2xl font-black transition-all duration-700 ${isHighlighted ? 'text-indigo-700 dark:text-indigo-400 scale-105' : 'text-slate-800 dark:text-white'}`}>{value} <span className="text-xs sm:text-sm font-bold text-slate-400">{unit}</span></p>}
      </div>
      <div className={`${bgClass} p-2.5 sm:p-3 rounded-xl shrink-0 transition-all duration-700 ${isHighlighted ? 'scale-125' : ''}`}><Icon className="w-5 h-5 sm:w-6 sm:h-6" /></div>
    </div>
  );
}
...
export default function AdminReports() {
  ...
  // 🟢 AI Chat History (Database) States
  ...
  const loadChat = async (id) => {
    ...
    if (data && data.messages) {
      const msgsWithIds = data.messages.map((m, i) => m.id ? m : { ...m, id: Date.now().toString() + i });
  ...
  // 🟢 ไฮไลท์นานขึ้น & อัปเดต UI แบบ Real-time ตามจังหวะที่พูด
  const speakSegments = async (segments) => {
    ...
    for (const segment of segments) {
      ...
      // 1. ชี้เป้า Highlight และเลื่อนจอให้ตรงจุด
      ...
      // 2. 🟢 สั่งเปลี่ยน UI หน้าเว็บ (ตัวกรองต่างๆ) ให้สัมพันธ์กับประโยคที่กำลังพูด
      if (segment.action) {
        let changed = false;
        if (segment.action.filterMonth !== undefined) { setFilterMonth(segment.action.filterMonth); changed = true; }
        if (segment.action.histMonth !== undefined) { setHistMonth(segment.action.histMonth); changed = true; }
        if (segment.action.histType !== undefined) { setHistType(segment.action.histType); changed = true; }
        if (segment.action.histSearch !== undefined) { setHistSearch(segment.action.histSearch); changed = true; }
        
        if (changed) {
          setHistPage(1);
          // ให้เวลา React อัปเดต UI หน้าจอเล็กน้อย (100ms) ก่อนเริ่มพูด
          await new Promise(r => setTimeout(r, 100)); 
        }
      }

      // 3. เริ่มพูดประโยคนั้นๆ
      await new Promise((resolve) => {
        ...
        // กันระบบเสียงค้าง ไฮไลท์ก็ยังทำงานต่อไปได้
        setTimeout(safeResolve, Math.max(1500, segment.text.length * 70));
      });
    }
    ...
    if (!stopSpeakingRef.current) {
      setIsSpeaking(false);
      setBotStatus('idle');
      setHighlightId(''); // เคลียร์การไฮไลท์เมื่อพูดจบ "ทั้งหมด" แล้ว
    }
  };
  ...
  const startListening = () => {
    ...
    if (!SpeechRecognition) {
      toast.error("เบราว์เซอร์ของคุณไม่รองรับการสั่งงานด้วยเสียง แนะนำให้ใช้ Google Chrome ค่ะ");
      return;
    }
  ...
  const handleSendChat = async () => {
    ...
    const userMessageObj = { id: Date.now().toString() + '-user', role: 'user', text: userMessage };
    ...
    try {
      // 🟢 เตรียมข้อมูลที่แม่นยำ 100% ส่งให้ AI สรุป
      const monthlySummaries = THAI_MONTHS.map(m => {
        const mDeps = deposits.filter(d => new Date(d.deposit_date).getMonth() + 1 === parseInt(m.val) && new Date(d.deposit_date).getFullYear().toString() === filterYear);
        const w = mDeps.reduce((sum, d) => sum + Number(d.weight_kg), 0);
        const co2 = mDeps.reduce((sum, d) => sum + (Number(d.weight_kg) * (co2Map[d.waste_type] || 0)), 0);
      ...
      const wasteTypeSummaries = wasteTypes.map(t => {
         const tDeps = deposits.filter(d => d.waste_type === t.name && new Date(d.deposit_date).getFullYear().toString() === filterYear);
         const w = tDeps.reduce((sum, d) => sum + Number(d.weight_kg), 0);
         return { type: t.name, co2_factor_per_kg: t.co2_factor, total_weight_kg: w.toFixed(2), total_co2_reduced: (w * t.co2_factor).toFixed(2) };
      ...
      const contextPayload = {
        ...
        wasteTypeSummaries: wasteTypeSummaries
      };
      ...
      const availableHighlights = [
        ...
        { id: "history-table", description: "ตารางประวัติการฝากขยะล่าสุด" }
      ];
      ...
      const res = await fetch('/api/ai-chat', {
        ...
        body: JSON.stringify({ 
          ...
          pageName: "หน้ารายงานสถิติ (Reports)"
        })
      });
      ...
      if (data.speechSegments && data.speechSegments.length > 0) {
        ...
        const aiMessageObj = { 
          ...
          role: 'ai', 
          text: fullReply, 
          segments: data.speechSegments // เก็บ segments ไว้สำหรับกด "เล่นซ้ำ"
        };
        ...
        if (!currentChatId) {
          ...
          const { data: insertedChat } = await supabase.from('ecobot_chats').insert([{ title: newTitle, messages: finalMessages }]).select('id').single();
          if (insertedChat) {
            setCurrentChatId(insertedChat.id);
            fetchChatList();
          }
        } else {
  ...
  const groupedChats = useMemo(() => {
    ...
    return chatList.filter(c => c.title.toLowerCase().includes(chatSearch.toLowerCase())).reduce((acc, chat) => {
    ...
    <div className="space-y-4 sm:space-y-6 p-2 sm:p-0 relative overflow-x-hidden transition-colors">
        ...
        <div className="p-4 sm:p-6 lg:p-8 border-b border-slate-100 dark:border-slate-800 flex flex-col gap-3 bg-white/50 dark:bg-slate-900/50 backdrop-blur-md shrink-0 transition-colors">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            ...
            <div className="flex flex-wrap items-center gap-2 bg-slate-50/80 dark:bg-slate-950/50 p-2 rounded-xl border border-slate-200 dark:border-slate-800 transition-colors">
              ...
              <select value={filterMonth} onChange={e => setFilterMonth(e.target.value)} className="px-2 py-1.5 text-xs sm:text-sm font-bold border border-slate-200 dark:border-slate-700 rounded-lg outline-none bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 flex-1 sm:flex-none transition-colors"><option value="all">{t('allYearOverView')}</option>{THAI_MONTHS.map(m => <option key={m.val} value={m.val}>{lang === 'th' ? m.label : m.val}</option>)}</select>
              <select value={filterYear} onChange={e => setFilterYear(e.target.value)} className="px-2 py-1.5 text-xs sm:text-sm font-bold border border-slate-200 dark:border-slate-700 rounded-lg outline-none bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 flex-1 sm:flex-none transition-colors">{availableYears.map(y => <option key={y} value={y}>{y}</option>)}</select>
              ...
              <button onClick={exportPDF} disabled={processing} className="px-3 sm:px-5 py-1.5 sm:py-2 bg-slate-800 dark:bg-emerald-600 text-white rounded-lg sm:rounded-xl hover:bg-slate-700 dark:hover:bg-emerald-500 flex items-center gap-1.5 font-bold transition-all shadow-md active:scale-95 disabled:opacity-50 text-xs sm:text-sm whitespace-nowrap"><Download className="w-3.5 h-3.5 sm:w-4 sm:h-4" />{processing ? t('preparing') : t('exportPdf')}</button>
            </div>
          </div>
        </div>
                ...
                <div className="p-3 sm:p-4 h-48 sm:h-64">
                  {loading ? <Skeleton className="w-full h-full rounded-xl dark:bg-slate-800" /> : lineData.length === 0 ? <div className="flex h-full items-center justify-center text-slate-400 font-bold text-sm">{t('noData')}</div> :
                    <ResponsiveContainer width="100%" height="100%" minWidth={1}>
                      <LineChart data={lineData}><CartesianGrid strokeDasharray="3 3" stroke={theme === 'dark' ? '#334155' : '#f1f5f9'} /><XAxis dataKey="d" tick={{ fontSize: 10, fontWeight: 700, fill: theme === 'dark' ? '#94a3b8' : '#64748b' }} /><YAxis tick={{ fontSize: 10, fill: theme === 'dark' ? '#94a3b8' : '#64748b' }} /><Tooltip formatter={v => [`${v} ${t('colWeight')}`, t('chartLineTitle')]} contentStyle={{ backgroundColor: theme === 'dark' ? '#1e293b' : '#fff', borderColor: theme === 'dark' ? '#334155' : '#e2e8f0', color: theme === 'dark' ? '#fff' : '#000' }} /><Line type="monotone" dataKey="w" stroke="#3b82f6" strokeWidth={2.5} dot={false} /></LineChart>
                    </ResponsiveContainer>
                  }
                </div>
              </div>
              ...
              <div id="chart-pie" className={`bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden transition-all duration-700 ease-in-out ${highlightId === 'chart-pie' ? 'ring-[6px] ring-indigo-500 ring-offset-4 dark:ring-offset-slate-950 scale-[1.06] z-30 relative shadow-[0_0_40px_rgba(99,102,241,0.4)]' : 'scale-100'}`}>
                ...
                <div className="p-2 sm:p-4 h-48 sm:h-64 flex items-center justify-center">
                  {loading ? <Skeleton className="w-36 h-36 rounded-full dark:bg-slate-800" /> : pieData.length === 0 ? <div className="text-slate-400 font-bold text-sm">{t('noData')}</div> :
                    <ResponsiveContainer width="100%" height="100%" minWidth={1}>
                      <PieChart><Pie data={pieData} dataKey="w" nameKey="name" cx="50%" cy="50%" outerRadius="65%" innerRadius="30%">{pieData.map((_, i) => <Cell key={i} fill={PIE_COLORS[i % PIE_COLORS.length]} />)}</Pie><Tooltip formatter={v => [`${v} ${t('colWeight')}`]} contentStyle={{ backgroundColor: theme === 'dark' ? '#1e293b' : '#fff', borderColor: theme === 'dark' ? '#334155' : '#e2e8f0', color: theme === 'dark' ? '#fff' : '#000' }} /><Legend iconSize={8} wrapperStyle={{ fontSize: '10px', fontWeight: 700, color: theme === 'dark' ? '#cbd5e1' : '#475569' }} /></PieChart>
                    </ResponsiveContainer>
                  }
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-5">
              <div id="top-users" className={`bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden transition-all duration-700 ease-in-out ${highlightId === 'top-users' ? 'ring-[6px] ring-indigo-500 ring-offset-4 dark:ring-offset-slate-950 scale-[1.04] z-30 relative shadow-[0_0_40px_rgba(99,102,241,0.4)]' : 'scale-100'}`}>
                ...
                <div className="divide-y divide-slate-50 dark:divide-slate-800/50">
                  {loading ? Array.from({ length: 5 }).map((_, i) => <div key={i} className="px-4 py-3 flex items-center gap-3"><Skeleton className="w-6 h-6 rounded-full shrink-0 dark:bg-slate-800" /><Skeleton className="h-4 flex-1 rounded-lg dark:bg-slate-800" /><Skeleton className="h-4 w-14 rounded-lg dark:bg-slate-800" /></div>)
                  : topUsers.map((user, i) => (
              ...
              <div id="history-table" className={`lg:col-span-2 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden transition-all duration-700 ease-in-out ${highlightId === 'history-table' ? 'ring-[6px] ring-indigo-500 ring-offset-4 dark:ring-offset-slate-950 scale-[1.02] z-30 relative shadow-[0_0_40px_rgba(99,102,241,0.4)]' : 'scale-100'}`}>
                <div className="px-4 py-2.5 bg-cyan-500 text-white">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                    ...
                    <div className="flex flex-wrap gap-1.5 items-center">
                      ...
                      <select value={histType} onChange={e => { setHistType(e.target.value); setHistPage(1); }} className="px-1.5 py-1 text-xs font-bold border border-white/20 rounded-lg bg-white/10 text-white outline-none max-w-[90px] [&>option]:text-slate-800"><option value="">{t('colType')}</option>{wasteTypeNames.map(t => <option key={t} value={t}>{t}</option>)}</select>
                ...
                {histTotalPages > 1 && (
                  <div className="px-4 py-3 border-t border-slate-100 dark:border-slate-800 flex justify-between gap-2 bg-white dark:bg-slate-900">
                    <p className="text-xs font-bold text-slate-400 dark:text-slate-500">{t('pageText')} {histPage} / {histTotalPages}</p>
                    <div className="flex gap-1"><button disabled={histPage===1} onClick={() => setHistPage(p=>p-1)} className="px-2.5 py-1 text-xs border dark:border-slate-700 rounded hover:bg-slate-50 dark:hover:bg-slate-800 disabled:opacity-40 text-slate-600 dark:text-slate-300">←</button><button disabled={histPage===histTotalPages} onClick={() => setHistPage(p=>p+1)} className="px-2.5 py-1 text-xs border dark:border-slate-700 rounded hover:bg-slate-50 dark:hover:bg-slate-800 disabled:opacity-40 text-slate-600 dark:text-slate-300">→</button></div>
                  </div>
                )}
              </div>
            </div>
      ...
      {isChatOpen && (
        <div className={`fixed z-50 bottom-24 right-6 w-[320px] sm:w-[380px] bg-white dark:bg-slate-900 shadow-2xl rounded-2xl flex flex-col overflow-hidden transition-all duration-300 ${isSpeaking ? 'speaking-border' : 'border border-slate-200 dark:border-slate-800'}`} style={{ height: '500px' }}>
          ...
          {/* 🟢 เมนูประวัติแชท (Sidebar Overlay) */}
          <AnimatePresence>
            {isChatMenuOpen && (
              <motion.div initial={{ x: '-100%' }} animate={{ x: 0 }} exit={{ x: '-100%' }} transition={{ type: "tween", duration: 0.2 }} className="absolute inset-0 z-30 bg-white dark:bg-slate-900 flex flex-col shadow-[4px_0_15px_rgba(0,0,0,0.1)]">
                <div className="p-3 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-950">
                  ...
                  <button onClick={() => setIsChatMenuOpen(false)} className="p-2 bg-slate-200 dark:bg-slate-800 rounded-full hover:bg-slate-300 dark:hover:bg-slate-700 transition-colors text-slate-600 dark:text-slate-300"><X className="w-4 h-4" /></button>
                </div>
                ...
                <div className="flex-1 overflow-y-auto p-2 scrollbar-hide space-y-4">
                  {Object.entries(groupedChats).map(([group, chats]) => chats.length > 0 && (
                    <div key={group}>
                      <p className="text-[10px] font-black uppercase text-slate-400 ml-2 mb-1.5">{t(group)}</p>
                      <div className="space-y-1">
                        {chats.map(chat => (
                          <div key={chat.id} onClick={() => loadChat(chat.id)} className={`group relative p-2 rounded-lg cursor-pointer transition-colors ${currentChatId === chat.id ? 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-400' : 'hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300'}`}>
                            <p className="text-xs font-bold truncate pr-6">{chat.title}</p>
                            <button onClick={(e) => deleteChatGroup(chat.id, e)} className="absolute right-2 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 p-1 text-slate-400 hover:text-red-500 transition-all"><Trash2 className="w-3 h-3" /></button>
                          </div>
                        ))}
                      </div>
                    </div>
          ...
          <div className="bg-gradient-to-r from-slate-800 to-slate-700 dark:from-slate-950 dark:to-slate-900 px-3 py-3 flex items-center justify-between text-white shrink-0 border-b border-slate-800">
            ...
            <div className="flex items-center gap-1">
              {isSpeaking && (
                <button onClick={stopAiSpeech} className="p-1.5 bg-red-500/20 text-red-300 hover:bg-red-500 hover:text-white rounded-lg transition-colors">
              ...
              {chatAlerts.slice(-3).map((alert) => (
                <div key={alert.id} className={`flex items-start gap-2 px-2.5 py-1.5 rounded-lg text-xs font-semibold border ${
                  ...
                  {alert.type === 'success' ? <CheckCircle2 className="w-3.5 h-3.5 shrink-0 mt-0.5" /> :
                   alert.type === 'error'   ? <AlertCircle className="w-3.5 h-3.5 shrink-0 mt-0.5" /> :
            ...
            {chatMessages.map((msg, idx) => (
              <div key={msg.id || idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                ...
                <div className="flex flex-col gap-1 max-w-[80%]">
                  ...
                  {/* 🟢 ปุ่มเล่นซ้ำ และ ปุ่มลบ (จะแสดงเฉพาะข้อความที่มี ID) */}
                  <div className={`flex items-center gap-2 mt-0.5 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                    {msg.role === 'ai' && msg.segments && msg.segments.length > 0 && (
                      <button
          ...
          <div className="p-2.5 bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 flex items-center gap-2 shrink-0">
            <button onClick={startListening} className={`p-2 rounded-full shrink-0 transition-all ${isListening ? 'bg-red-100 text-red-500 animate-pulse' : 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700'}`}>
            ...
            <input
              ...
              placeholder={isListening ? "กำลังฟังคำสั่ง..." : t('typeCommand')}
